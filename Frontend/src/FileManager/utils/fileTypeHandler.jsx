...
export const getFileIcon = (fileName, size = 20) => {
  // Add null/undefined check for fileName
  if (!fileName) {
 ...
 * @param {boolean} params.isOnHold - Whether account is on hold
 */
export const handleFileOpen = async ({ fileName, selectedPath, projectId, axiosSecure, mode = "preview", isOnHold = false }) => {
  // Add safety check for fileName
  if (!fileName) {
    console.error("❌ handleFileOpen: fileName is missing or undefined", { fileName, selectedPath, projectId, mode });
    return false;
  }
  ...
  if (ext === 'eml' || ext === 'msg') {
    await handleEmailFile({ fileName, selectedPath, projectId, axiosSecure, isOnHold });
    return true; // Return true to indicate the file was handled
  }
  
  // For all other files, let the caller handle it (return false so SortableFile handles it)
  return false;
};
 ...
 * @param {boolean} params.isOnHold - Whether account is on hold
 */
const handleEmailFile = async ({ fileName, selectedPath, projectId, axiosSecure, isOnHold = false }) => {
  try {
    // Check if account is on hold before downloading
    if (isOnHold) {
      // Import Swal and show warning
      const SwalModule = await import('sweetalert2');
      const Swal = SwalModule.default;
      Swal.fire({
        title: "Downloads Disabled",
        text: "Email file downloads are disabled while your account is on hold. Please contact support or resolve outstanding invoices to restore download access.",
        icon: "warning",
