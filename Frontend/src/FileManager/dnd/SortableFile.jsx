...
const SortableFile = ({
  ...
  console.log(`📄 SortableFile "${fileName}" - isOnHold:`, isOnHold);
  ...
  const handleDownload = (e) => {
    ...
    console.log(`⬇️ Download clicked for "${fileName}" - isOnHold:`, isOnHold);
  ...
  const handlePreview = async (e) => {
    ...
    const handled = await handleFileOpen({
      fileName,
      axiosSecure,
      selectedPath,
      projectId,
    ...
    if (!handled) {
      ...
      if (previewable.includes(ext)) {
        ...
        // Check if account is on hold before offering download
        if (isOnHold) {
          Swal.fire({
            title: "Downloads Disabled",
            text: "File downloads are disabled while your account is on hold. This file type cannot be previewed. Please contact support or resolve outstanding invoices to restore download access.",
            icon: "warning",
            confirmButtonText: "OK"
          });
          return;
    ...
    >
      ...
      <div className="flex gap-2 items-center">
        ...
        <button
          ...
          disabled={isOnHold}
          className={`invisible group-hover:visible p-1 rounded transition ${
            isOnHold 
              ? 'cursor-not-allowed opacity-50' 
              : 'hover:bg-blue-100'
          }`}
          title={isOnHold ? "Downloads disabled while account is on hold" : "Download"}
        >
          <IconDownload size={16} className={isOnHold ? "text-gray-400" : "text-blue-600"} />
        </button>
