...
/*  const currentSubfolders = Object.fromEntries(
 ...
 // remove "__meta", and if we’re at the root, hide Admin/Estimator for non-admins
 const currentSubfolders = Object.fromEntries(
   Object.entries(rawSubfolders)
     .filter(([key]) => key !== "__meta")
     .filter(([key]) => {
       // if we’re in the root folder…
       if (normalizedSelectedPath === ".") {
         // and the user is NOT an admin…
         if (userRole !== "Admin") {
           // hide these two folders
           return key !== "Admin" && key !== "Estimator";
         }
       }
       return true;
     })
    ...
    <div className="relative flex flex-1 overflow-hidden">
      ...
      <div className="hidden sm:flex w-1/3 bg-gray-50 border-r flex-col justify-between">
       ...
       <FolderSidebar
         ...
         folders={folderList.filter((f) => {
           if (selectedPath === "." && userRole !== "Admin") {
             return f !== "Admin" && f !== "Estimator";
           }
           return true;
         })}
            selectedPath={normalizedSelectedPath}
