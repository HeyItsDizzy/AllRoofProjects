...
const visibleFiles = liveFiles.filter((fileName) => !fileName.startsWith("."));
  ...
  <button
    disabled={isOnHold}
    title={isOnHold ? "Downloads disabled while account is on hold" : `Download ${currentFolderLabel} as ZIP`}
    aria-label={`Download ${currentFolderLabel} as ZIP`}
    className={`border px-4 py-[6px] rounded-md flex items-center justify-center text-sm h-[32px] transition ${
      isOnHold 
        ? 'border-gray-300 text-gray-400 cursor-not-allowed' 
        : 'border-green-500 text-green-600 cursor-pointer hover:bg-green-200'
    }`}
    onClick={() => {
      if (isOnHold) {
        Swal.fire({
      ...
      downloadToZIP({ axiosSecure, selectedPath, projectId });
    }}
  >
    <IconDownload size={25} className={isOnHold ? "text-gray-400 mr-1" : "text-green-500 mr-1"} />
    {`${currentFolderLabel}`}
  </button>
</div>
