...
export const QuickBooksConnectButton = ({ 
  ...
  const { authenticate, isLoading, error } = useQuickBooksAuth();
...
export const QuickBooksCallback = ({ onSuccess, onError }) => {
  const { handleCallback } = useQuickBooksAuth();
  ...
  useEffect(() => {
    const processCallback = async () => {
      try {
        ...
        const tokenData = await handleCallback(code, realmId, state, state);
        onSuccess?.(tokenData);
