...
export const useQuickBooksAuth = (config = {}) => {
   ...
   */
  const authenticate = useCallback((scopes = [client.scopes.Accounting, client.scopes.OpenId]) => {
    try {
      setError(null);
      ...
      localStorage.setItem('quickbooks_oauth_state', authUrl.split('state=')[1]?.split('&')[0] || '');
