...
class QuickBooksClient {
  constructor(config = {}) {
    ...
    this.environment = config.environment || process.env.REACT_APP_QB_ENVIRONMENT || 'sandbox';
    this.redirectUri = config.redirectUri || process.env.REACT_APP_QB_REDIRECT_URI || `${window.location.origin}/quickbooks/callback`;
  ...
  getAuthorizationUri(options = {}) {
    if (!options.scope) {
      throw new Error('Scopes are required for QuickBooks authorization');
    }

    const scopes = Array.isArray(options.scope) ? options.scope.join(' ') : options.scope;
