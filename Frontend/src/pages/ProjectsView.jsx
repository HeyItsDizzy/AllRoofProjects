...
const ProjectsView = () => {
  ...
  const { isFeatureBlocked, isOnHold } = useClientAccountStatus();
  
  console.log('🎯 ProjectsView - Account Status:', { 
    isOnHold, 
    userRole: user?.role,
  ...
  const fetchProjectData = useCallback(async () => {
    ...
    try {
      ...
      if (!fetchedProject.linkedUsers?.includes(user?.id) && userRole === "User") {
        fetchedProject.linkedUsers = fetchedProject.linkedUsers || [];
  ...
  const handleFolderRename = useCallback(async () => {
    try {
      ...
      // Non-critical error - disk folder rename is cosmetic only
      // Project data has been successfully updated in the database
    }
  }, [project, axiosSecure]);
    ...
    <div className="min-h-screen pb-4 mt-4">
      ...
      <div ref={componentRef}>
        ...
        >
          ...
          {/* Folder Panel Wrapper - Always visible, but downloads/uploads blocked when on hold */}
          <FolderPanelWrapper
          ...
          <div className="bg-white p-4 rounded-md mb-4 shadow-sm mt-6">
            <ScopeRequestDropZone 
              ...
              onProcessingComplete={(result) => {
                // Refresh folder panel to show new files
                setFolderPanelRefreshKey(prev => prev + 1);
