...
const SendEstimateModal = ({ isVisible, onClose, project }) => {
  ...
  useEffect(() => {
    if (isVisible) {
      console.log('[SendEstimateModal] is open');
    }
  }, [isVisible]);
  ...
  const loadPDFsFromBOQ = async () => {
    ...
    try {
      ...
      if (response.data && response.data.folders) {
        // Find BOQ folder
        const boqFolder = response.data.folders.find(folder => folder.name === 'BOQ');
        
        if (boqFolder && boqFolder.files) {
          // Filter only PDF files
          const pdfFiles = boqFolder.files.filter(file => 
    ...
    try {
      ...
      if (sendToLinkedClient) {
        ...
        emailData = {
          ...
          attachments: attachPDFs && selectedPDFs.length > 0 ? selectedPDFs.map(pdf => ({
            folderPath: 'BOQ',
            fileName: pdf.name
          })) : []
        };
                          ...
                          <Checkbox
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                ...
                                setSelectedPDFs(selectedPDFs.filter(p => p.name !== pdf.name));
                              }
                            }}
                          />
                          <span className="ml-2 text-sm text-gray-700 flex-1 truncate" title={pdf.name}>
                            {pdf.name}
                          </span>
