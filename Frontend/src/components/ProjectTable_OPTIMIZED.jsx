/**
 * PROJECT TABLE COMPONENT - OPTIMIZED HYBRID APPROACH ✅
 * 
 * PERFORMANCE STRATEGY:
 * ✅ Smart loading: Fetch recent 200 projects initially (fast!)
 * ✅ Lazy-load older projects only when "Older" tab is clicked
 * ✅ Client-side pagination for instant page navigation
 * ✅ Skeleton loading prevents CLS (Cumulative Layout Shift)
 * ✅ Month counts work properly with loaded data
...
export default function ProjectTable({
  ...
  // ══════════════════════════════════════════════════════════════════
  // 🎯 SMART LOADING STATE - Optimized for Performance
  // ══════════════════════════════════════════════════════════════════
  const [allProjects, setAllProjects] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [isLoadingOlder, setIsLoadingOlder] = useState(false);
  const [hasLoadedAllProjects, setHasLoadedAllProjects] = useState(false);
  ...
  // Track if we've already loaded older projects
  const hasTriedLoadingOlder = useRef(false);

  // ══════════════════════════════════════════════════════════════════
  // 🚀 OPTIMIZED FETCH - Load Recent Projects First (FAST!)
  // ══════════════════════════════════════════════════════════════════
  const fetchRecentProjects = useCallback(async () => {
    try {
      ...
      console.log('🚀 FAST LOAD: Fetching recent 200 projects for instant display...');

      // Fetch only recent projects (last 6 months or 200 projects, whichever is less)
      const response = await axiosSecure.get("/projects/get-projects", {
        params: {
          limit: 200, // Reasonable amount for fast loading
      ...
      if (response.data.success) {
        ...
        setHasLoadedAllProjects(false); // Haven't loaded everything yet
        console.log(`✅ FAST LOAD: Loaded ${projectsData.length} recent projects (instant display)`);
      } else {
  ...
  // ══════════════════════════════════════════════════════════════════
  // 📦 LAZY LOAD - Load All Projects Only When Needed
  // ══════════════════════════════════════════════════════════════════
  const fetchAllProjects = useCallback(async () => {
    if (hasLoadedAllProjects || hasTriedLoadingOlder.current) {
      console.log('✅ All projects already loaded, skipping...');
      return;
    }

    try {
      setIsLoadingOlder(true);
      hasTriedLoadingOlder.current = true;

      console.log('📦 LAZY LOAD: Fetching ALL projects (for older data access)...');
      ...
      if (response.data.success) {
        const projectsData = response.data.projects || response.data.data || [];
        setAllProjects(projectsData);
        setHasLoadedAllProjects(true);
        console.log(`✅ LAZY LOAD: Loaded all ${projectsData.length} projects successfully`);
      }
    } catch (error) {
      console.error('❌ Error fetching all projects:', error);
      // Don't show error - user can still work with recent projects
    } finally {
      setIsLoadingOlder(false);
    }
  }, [axiosSecure, hasLoadedAllProjects]);
  ...
  // Handle month tab changes - LAZY LOAD TRIGGER
  const handleMonthTabChange = useCallback((tabId) => {
    console.log(`📅 Month tab changed: ${tabId}`);
    setActiveTab(tabId);
    setCurrentPage(1); // Reset to first page when changing tabs

    // Lazy load all projects if user clicks on "Older" or an older month
    if (tabId === 'older' || (tabId !== 'all' && tabId !== 'lastN' && !recentMonths.find(m => m.id === tabId))) {
      console.log('🔄 User accessing older data, triggering lazy load...');
      fetchAllProjects();
    }
  }, [recentMonths, fetchAllProjects]);

  // Handle older month selection from dropdown - LAZY LOAD TRIGGER
  const handleOlderMonthSelect = useCallback((monthId) => {
    setSelectedOlderMonth(monthId);
    console.log('🔄 Older month selected, ensuring all data loaded...');
    fetchAllProjects();
  }, [fetchAllProjects]);
  ...
  // ══════════════════════════════════════════════════════════════════
  // 📊 DATA FILTERING PIPELINE - Optimized for Performance
  // ══════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════
  // 🎨 MAIN RENDER - OPTIMIZED SKELETON LOADING (Prevents CLS)
  // ══════════════════════════════════════════════════════════════════

  if (loading) {
    return (
      <div className="w-full space-y-4">
        {/* Skeleton for Month Tabs */}
    ...
    <div className="w-full space-y-4">
      ...
      {isLoadingOlder && (
        <div className="flex items-center justify-center py-2 bg-blue-50 rounded-md">
          ...
          <span className="text-sm text-blue-600">Loading older projects...</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-600">
          ...
          {!hasLoadedAllProjects && (
            <span className="ml-2 text-xs text-blue-600">(Recent data loaded)</span>
          )}
        </div>
      </div>
