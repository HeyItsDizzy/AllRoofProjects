/**
 * Job Logged Email Template
 * Professional email template for notifying clients when their job has been logged
 */

/**
 * Generate job logged email
 ...
 * @param {string} data.dueDateText - Expected completion date text
 * @param {string} data.projectViewUrl - Direct URL to view the project with read-only token (optional)
 ...
 */
const jobLogged = (data) => {
  const { projectAddress, projectNumber, clientName, dueDateText, projectViewUrl, companyLogoUrl, optionalBody } = data;
  
  const subject = `Job Logged${projectNumber ? ` - Ref: ${projectNumber}` : ''}${projectAddress ? ` - ${projectAddress}` : ''}`;
...
<head>
  <meta charset="utf-8">
  <title>Job Logged${projectNumber ? ' - ' + projectNumber : ''}</title>
...
<body style="margin:0; padding:0; background-color:#F3F4F6; font-family:Arial, sans-serif; line-height:1.6; color:#081F13;">
  ...
  <div style="display:none; font-size:1px; line-height:1px; max-height:0; max-width:0; opacity:0; overflow:hidden; mso-hide:all;">
    Your job has been logged${dueDateText ? ` and is expected by the morning of ${dueDateText}` : ''}.
  </div>
          ...
          <tr>
            <td style="padding:28px 24px 8px 24px;">
              ...
              <p style="margin:0 0 12px 0; color:#4B5563; font-size:15px; line-height:1.6;">
                Your request for <strong style="color:#009245;">${projectAddress}</strong> has been logged${projectNumber ? ' (Ref #'+projectNumber+')' : ''}.
              </p>` : ''}

              <p style="margin:0 0 14px 0; color:#4B5563; font-size:15px; line-height:1.6;">
                You can expect this to be<br>
                <strong style="color:#009245;">Completed by the morning of ${dueDateText || 'TBD'}</strong>.
              </p>
              ...
              <!-- Safety plain link -->
              <p style="margin:6px 0 18px 0; color:#9CA3AF; font-size:12px; line-height:1.5;">
                If the button doesn't work, copy and paste this link into your browser:<br>
                <span style="word-break:break-all; color:#6B7280;">${projectViewUrl}</span>
              </p>` : ''}
