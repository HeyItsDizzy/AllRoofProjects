...
class EmailService {
  ...
  async sendCompanyInvitation(email, data) {
    const subject = `Company Invitation - ${data.companyName || 'Join Our Platform'}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>Company Invitation</h2>
        <p>Dear ${data.contactName || 'User'},</p>
        <p>You have been invited to join ${data.companyName || 'our platform'}.</p>
        ${data.invitationLink ? `<p><a href="${data.invitationLink}">Accept Invitation</a></p>` : ''}
        <p>Best regards,<br>All Roof Take-Offs</p>
      </div>
    `;
